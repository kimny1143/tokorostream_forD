...
def resample_audio(input_file, target_sr=44100):
    try:
        ...
        if original_sr != target_sr:
            ...
            logging.info(f"Resampled audio {input_file} from {original_sr} to {target_sr}")
...
def process_audio_file(file_path, sources, model, device, output_dir):
    try:
        logging.info(f"Processing audio file {file_path} with model {model} on {device}")
        ...
        for i, source_name in enumerate(['vocals', 'drums', 'bass', 'other']):
            if source_name in sources:
                ...
                logging.info(f"Saved separated source {source_name} to {output_path}")
    except Exception as e:
